...
public class TerminationDAO {
    ...
    private static final String UPDATE_TERMINATION = "update tbl_termination set em_no=?,termination_date=?, notice_date=?,termination_reason=?,voluntary_termination=? where t_id=?";
    private static final String SELECT_ALL_TERMINATION = "SELECT  t.t_id,  CONCAT(ifnull(e.first_name,'' ), ' ',ifnull( e.other_name,'' ), ' ',ifnull( e.surname,'' )  )as EName,  t.termination_date,  t.notice_date,  t.voluntary_termination, t.termination_reason FROM tbl_termination t JOIN emp_bio e ON e.emp_no=t.emp_no";
    private static final String SELECT_TERMINATION_BY_ID = "SELECT  t.t_id,  CONCAT(ifnull(e.first_name,'' ), ' ',ifnull( e.other_name,'' ), ' ',ifnull( e.surname,'' )  )as EName,  t.termination_date,  t.notice_date,  t.voluntary_termination, t.termination_reason FROM tbl_termination t JOIN emp_bio e ON e.emp_no=t.emp_no where t_id=?";
    ...
    public List<TerminationModel> getAllTerminationModels() {
        ...
        try {
            ...
            while (conn.rs.next()) {
                ...
                terminate.setT_id(conn.rs.getString("t_id"));
